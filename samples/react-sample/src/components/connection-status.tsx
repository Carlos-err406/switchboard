...
export function ConnectionStatus({
  ...
  if (state.isWebSocketConnected)
    return (
      <Badge variant="outline" className="border-green-600 text-green-600 p-1">
        connected
      </Badge>
    );
  if (state.hasEverConnected)
    return (
      <Badge
        variant="outline"
        className="border-yellow-600 text-yellow-600 p-1"
      >
        reconnecting ({state.connectionRetries})
      </Badge>
    );
  return (
    <Badge variant="outline" className="border-muted-foreground p-1">
